...
(define (eval-if exp env)
  ...
  (define (if-rec args)
    (cond ((tagged-list? args 'else) (cadr args))
          ((true? (mc-eval (cadr args) env)) (cadddr args))
          (else (if-rec (cddddr args)))))
...
(define (and-thing exp env)
  
  (define (and-rec args)
    (cond ((null? args) #t)
          ((false? (mc-eval (car args) env)) #f)
          (else (and-rec (cdr args)))))
  
  (and-rec (and-predicates exp)))
...
(define (or-thing exp env)

  (define (or-rec args)
    (cond ((null? args) #f)
          ((true? (mc-eval (car args) env)) #t)
          (else (or-rec (cdr args)))))
  
  (or-rec (or-predicates exp)))
...
(define (let-thing exp)

  (define (let-iter args varlst explst)
    (if (null? (cdr args))
        (append (list (make-lambda varlst args))
                explst)
        (let-iter (cddddr args)
                  (append varlst (list (car args)))
                  (append explst (list (caddr args))))))
...
(define (let-thing exp)
  ...
  (define (let-iter args varlst explst)
    (if (null? args)
        (append (list (make-lambda varlst (list (let-body exp))))
                explst)
        (let-iter (cdr args)
                  (append varlst (list (caar args)))
                  (append explst (list (cadar args))))))
...
((lambda varlst (let-body exp)) explst)
...
list2 = (exp1 exp2 exp3)

(cons (lambda list1 body) list2)
